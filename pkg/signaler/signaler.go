...
const (
	New       Method = "new"
	Callme    Method = "callme"
	switchCamera Method = "switchCamera"
	Bye       Method = "bye"
	Offer     Method = "offer"
...
var callmeData struct {
	ToID  string `json:"toid"`  // 发送者的 ID
	TzID  string `json:"tzid"`  // 目标用户的 ID
}
...
func (s *Signaler) HandleNewWebSocket(conn *websocket.WebSocketConn, request *http.Request) {
	...
	conn.On("message", func(message []byte) {
		...
		case Callme:
			// 解析数据中的目标ID（toid）
			if err := json.Unmarshal(body, &callmeData); err != nil {
				logger.Errorf("Unmarshal callme data error: %v", err)
				return
			}
			targetPeerID := callmeData.TzID // 需要转发的目标用户 ID
			// 查找目标peer
		...
		case switchCamera:
			...
			if err := json.Unmarshal(body, &switchCameraData); err != nil {
				logger.Errorf("Unmarshal switchCamera data error: %v", err)
				return
			}
			targetPeerID := switchCameraData.SwId
			targetPeer, ok := s.peers[targetPeerID]
			if !ok {
				s.Send(conn, Request{
					Type: "error",
